...
const Comments = () => {
  ...
  const rootCommentsNoParent = comments.filter(
    backend => backend.parentId === null,
  );
  const getReplies = commenId => {
    return comments
      .filter(backendComment => backendComment.parentId === commenId)
      .sort(
        (a, b) =>
  ...
  return (
    <div className="comments">
      ...
      {!isAuthenticated && (
        <>
          <div
            className="comment-form-title container "
            style={{ textAlign: 'center' }}>
            Login to comment
          </div>
          <Link to="/login">
            <button className="comment-form-button container">Login</button>
          </Link>
        </>
      )}
