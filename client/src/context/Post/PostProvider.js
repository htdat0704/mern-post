...
import {
    ...
    updateComment,
    deleteComment,
} from '../../reducers/PostReducer/postActions'
...
function PostProvider({ children }) {
    ...
    const deletePostt = async (postId) => {
        try {
            ...
            const response = await axios.delete(
                `${apiUrlHeroku}/post/delete/${postId}`
            )
            if (response.data.success) {
    ...
    const updateOnePost = async (updatePostForm) => {
        try {
            const response = await axios.put(
                `${apiUrlHeroku}/post/update/${updatePostForm._id}`,
                updatePostForm
            )
            if (response.data.success) {
    ...
    const addComment = async (body, parentId = null) => {
        try {
            ...
            const response = await axios.post(
                `${apiUrlHeroku}/post/createComment`,
                formComment
            )
            if (response.data.success) {
                await dispatch(getComment(response.data.commentFind))
    ...
    const deleteCommentId = async (commentId) => {
        try {
            await setAuthToken(localStorage[LOCAL_STORAGE_TOKEN_NAME])
            const response = await axios.delete(
                `${apiUrlHeroku}/post/deleteComment/${commentId}`
            )
            if (response.data.success) {
    ...
    const updateCommentId = async (text, commentId) => {
        try {
            ...
            const response = await axios.put(
                `${apiUrlHeroku}/post/updateComment/${commentId}`,
                updateCommentForm
            )
            if (response.data.success) {
    ...
    const getPostsByUser = async (username) => {
        try {
            const response = await axios.get(
                `${apiUrlHeroku}/post/getPostUser/${username}`
            )
            if (response.data.success) {
                dispatch(setPostSuccess(response.data.posts))
            }
