...
class PostController {
  async createPost(req, res, next) {
    ...
    if (!title) {
      return res
        .status(400)
        .json({ success: false, message: "Missing Tittle" });
    }
  ...
  updatePost = async (req, res) => {
    ...
    try {
      ...
      const postFind = await Post.findOne({ _id: req.params.id }).populate(
        "user",
        ["username"]
      );
  ...
  getOnePost = async (req, res) => {
    try {
      const postFind = await Post.findOne({ slug: req.params.slug }).populate(
        "user",
        ["username"]
      );
      const commentFind = await Comment.find({ post: postFind._id }).populate(
        "user",
        ["username"]
      );
  ...
  deleteComment = async (req, res) => {
    try {
      ...
      const findCommentDelete = await Comment.findOne({
        _id: req.params.id,
      }).lean();
      const deleteChildComment = await Comment.deleteMany({
        parentId: findCommentDelete._id,
      });
  ...
  getPostByUser = async (req, res) => {
    try {
      ...
      const posts = await Post.find({ user: userFound._id }).populate("user", [
        "username",
      ]);
      res.json({ success: true, posts });
    } catch (error) {
